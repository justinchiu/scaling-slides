...
\vspace{2em}
\item But most tasks are fully supervised
\vspace{2em}
\item Evaluation of unsupervised is also difficult
\vspace{2em}
\item Make latent variable models competitive on an existing task
\end{itemize}
\end{frame}
...
\vspace{2em}
\begin{itemize}
\item Start $\bm\pi\in[0,1]^Z$, with
    $$[\bm\pi]_{z_1} = p(x_1 \mid z_1)p(z_1)$$
\item Transition operators $\Lambda\in[0,1]^{Z \times Z}$, with
    $$[\Lambda_t]_{z_{t-1},z_t} = p(x_t \mid z_t)p(z_t \mid z_{t-1})$$
\item Forward algorithm computes evidence
$$p(x) = \underbrace{\bm\pi^\top\Lambda_2\Lambda_3}_{\alpha_3}\cdots\Lambda_T\bm1$$
