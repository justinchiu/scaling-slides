...
\item Fine-tuned on small data for downstream task
\end{itemize}
\vspace{1em}
...
\vspace{2em}
\item Language requires encoding long-range context
\end{itemize}
\vspace{2em}
\end{frame}
...
\vspace{2em}
\item Interpretable, but thought to be very poor language models\footcite{buys2018hmm}
\end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{Speed Comparison\footnote{16k state VL-HMM takes 2100 s/epoch}}
\begin{center}
\begin{figure}
...
\begin{itemize}
\item Extend to models with intractable exact inference
\item Examine the tradeoffs between 
\end{itemize}
\end{frame}
