...
\vspace{1em}
\item Focus on models that admit tractable inference (say to avoid error due to
    approximations during learning)
\vspace{1em}
\item Research question 1: Does uncertainty in the model improve performance?
\vspace{1em}
\item Research question 2: To what extent is the performance of tractable
...
To answer these questions, we focus on the task of language modeling
\vspace{2em}
\begin{itemize}
...
\vspace{2em}
\item Focus on the smallest dataset, Penn Treebank,
    and the simplest latent variable model
\end{itemize}
\vspace{2em}
PICTURE
\end{frame}
...
\vspace{2em}
    \begin{itemize}
    \item But still not as performant as fully observed models
    \end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Classical models for unsupervised tag induction in NLP
\vspace{2em}
\item 
\end{itemize}
\end{frame}
...
This yields the joint distribution $$p(x,z) = \prod_t p(x_t \mid z_t)p(z_t \mid z_{t-1}),$$
with distributional parameters
\begin{align*}
&\textnormal{start vector } & \pi\in[0,1]^{Z},
...
where we have the
\begin{align*}
&\textnormal{start, } & \alpha_1 &= \pi [O]_{\cdot,x_1},\\
...
i.e. $\alpha_3 = \alpha_1\Lambda_2\Lambda_3$ gives $p(z_3, x_{1:3})$
\end{frame}
...
\vspace{1em}
    ...
    \end{itemize}
\vspace{1em}
\item Kernel-based generalized softmax for transitions
\vspace{1em}
    \begin{itemize}
    \item[] \FancyUpArrow~Speed
    \end{itemize}
...
we use a nonlinear low-dimensional decomposition
\[
...
\]
\begin{itemize}
\item Fewer parameters than directly parameterizing $W$
\item Can further parameterize $U$ or $V = \textrm{MLP}(E_u)$
\item This particular parameterization does not affect inference
\end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{Block-Sparse Emission Constraints: Inference}
\begin{center}
