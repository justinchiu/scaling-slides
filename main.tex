...
Given the unnormalized emission logit matrix,
\begin{itemize}
    \item Mask out state-word entries in matrix
...
\begin{frame}
\frametitle{Low-Rank Decomposition}
Factor transition matrix $A$ into product of skinny matrices
\[
\vcenter{\hbox{\begin{tikzpicture}[baseline=-0.5ex]
    \draw (0,1.5) rectangle (1,2) node[pos=.5] {$\alpha^\top$};
\end{tikzpicture}}}
\times
\vcenter{\hbox{\begin{tikzpicture}[baseline=-0.5ex]
...
\vspace{1em}
\item Solution: Use a nonnegative decomposition
$$A = \phi(U)\phi(V)^\top,$$
with $\phi:\R^D\to\R_+$
\end{itemize}
\end{frame}
