...
(Give dimensions)
\begin{itemize}
\item Nonlinear low-dimensional decomposition
$$
\vcenter{\hbox{\begin{tikzpicture}[baseline=-0.5ex]
    \draw (0,0) rectangle (2,2) node[pos=.5] {$X$};
\end{tikzpicture}}}
\propto
\exp\left(\,
\vcenter{\hbox{\begin{tikzpicture}[baseline=-0.5ex]
...
\,\right)
$$
\item Fewer parameters than directly parameterizing $X$
\item Can further parameterize $U = \textrm{MLP}(E_u)$
\end{itemize}
\end{frame}
